...
public class F5BackpressureProbeApplication {
  ...
  static class ProbeEndpoints {
    ...
    public ProbeEndpoints() throws UnknownHostException {
      this.payload = InetAddress.getLocalHost().getCanonicalHostName();
    }
    ...
    void setStatus(@RequestBody Map<String, Object> state) {
      log.info("update health state: {}", state);
    ...
    @GetMapping("/payload")
    Mono<ResponseEntity<String>> payload() throws UnknownHostException {
      return
        Mono.delay(Duration.ofMillis(300))
          .then(Mono.just(conditionalResponse(payloadState, payload)));
    }
